...
export const mapwipemapvoteCommand = {
  data: new SlashCommandBuilder()
    .setName("mapwipemapvote")
    .setDescription(
      "Lance un vote de map pour le prochain mapwipe (Admin uniquement)"
    )
  ...
  async execute(interaction) {
    ...
    // 🧪 Mode test : 30 secondes au lieu du prochain mardi
    ...
    if (TEST_MODE) {
      ...
      const nextTuesday = getNextTuesday17h();
      endTime = nextTuesday;
      wipeTime = new Date(nextTuesday.getTime() + 60 * 60 * 1000); // Mardi 18h (1h après la fin du vote)
    }
    ...
    activeVotes.set(voteId, {
      ...
      voteMessageId: null,
      testMode: TEST_MODE,
      isMapwipe: true, // Identifier que c'est un vote de mapwipe
    });

    scheduleVoteEnd(voteId, endTime);

    const responseMessage = TEST_MODE
      ? "🧪 **MODE TEST** - MapWipe MapVote lancé avec succès ! (se termine dans 30 secondes)"
      : "✅ MapWipe MapVote lancé avec succès !";
...
// Fonction pour calculer le prochain mardi à 17h
function getNextTuesday17h() {
  const now = DateTime.now().setZone("Europe/Paris");
  let nextTuesday = now
    .startOf("week")
    .plus({ days: 1 }) // Mardi = jour 2 de la semaine (lundi = 1)
    .set({ hour: 17, minute: 0, second: 0 });

  // Si on est déjà passé le mardi 17h cette semaine, prendre le mardi suivant
  if (nextTuesday <= now) {
    nextTuesday = nextTuesday.plus({ weeks: 1 });
  }

  return nextTuesday.toJSDate();
}
...
async function sendVoteMessages(
  ...
  const announceEmbed = new EmbedBuilder()
    .setTitle(`${testModeIndicator}🌍 MAPWIPE MAP VOTE LANCÉ`)
    .setColor(TEST_MODE ? colors.YELLOW : colors.GREEN) // Couleur verte pour le mapwipe
    .addFields(
      {
        name: TEST_MODE ? "🧪 Test - Prochain mapwipe" : "🕐 Prochain mapwipe",
  ...
  const embeds = [
    ...images.map((img, i) =>
      new EmbedBuilder()
        .setImage(img)
        .setColor(TEST_MODE ? colors.YELLOW : colors.GREEN)
        .setURL(commonUrl)
    ),
  ...
  const voteEmbed = new EmbedBuilder()
    .setTitle(`${testModeIndicator}VOTEZ POUR LA PROCHAINE MAP (MAPWIPE)`)
    .setDescription(
